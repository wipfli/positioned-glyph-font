...
def generate_encoding(source):
    glyph_clusters = [] # most frequent last, least frequent first
    ...
    with open(downsampled_parameter_space_path) as f:
        for line in f:
            ...
            glyph_clusters.append(cluster)

    cluster_to_unicode_encoding = {}
    codepoint = -1
    for glyph in reversed(glyphs):
        codepoint = get_next_available_codepoint(codepoint)
        cluster_to_unicode_encoding[glyph] = codepoint

    return cluster_to_unicode_encoding

def write_encoding_csv(cluster_to_unicode_encoding):
    encoding_path = 'cluster_encoding.jsonl'
