...
void do_codepoint(fontstack *f, protozero::pbf_writer &parent, std::vector<FT_Face> &faces, FT_ULong char_code) {
    int i = -1;
    for (auto const &face : faces) {
        i++;

        std::string font_path = std::string((*(f->paths))[i]);

        FT_UInt char_index = 0;
        bool encoded_codepoint = false;        
        if (encoding_unicode_to_font.count(char_code) > 0) {
            if (encoding_unicode_to_font[char_code] == font_path) {
                char_index = encoding_unicode_to_index[char_code];
                encoded_codepoint = true;
            }
        }
        else {
            char_index = FT_Get_Char_Index(face, char_code);
            encoded_codepoint = false;
        }
        if (char_index > 0) {
            ...
            if (encoded_codepoint) {
                // std::cout << "top " << glyph.top << std::endl;
                // std::cout << "left " << glyph.left << std::endl;
                // std::cout << "advance " << glyph.advance << std::endl;

                glyph.top += encoding_unicode_to_y_offset[char_code];
                glyph.left += encoding_unicode_to_x_offset[char_code];
...
string do_range(fontstack *f, std::vector<FT_Face> &faces, std::string name, unsigned start, unsigned end) {
    string fontstack_data;
    {
        ...
        for (unsigned x = start; x <= end; x++) {
            ...
            do_codepoint(f, fontstack,faces, x);
        }
    }
...
extern "C" {
    fontstack *create_fontstack(const char *name) {
        ...
        f->paths = new std::vector<std::string>;
    ...
    glyph_buffer *generate_glyph_buffer(fontstack *f, uint32_t start_codepoint) {
        string result = do_range(f, *f->faces,*f->name,start_codepoint,start_codepoint+255);
